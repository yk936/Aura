...
export function categorizeError(err) {
  ...
  if (status === 401 || status === 403 || message.includes("401") || message.includes("403") || message.includes("api key") || message.includes("unauthorized") || message.includes("forbidden") || message.includes("auth")) {
    return ERROR_CATEGORIES.AUTH_FAILURE;
  }

  if (status === 408 || status === 504 || message.includes("timeout") || message.includes("econnreset") || message.includes("etimedout") || message.includes("network") || code.includes("etimedout")) {
...
export function getUserFacingErrorMessage(category, fallbackMsg = "") {
  switch (category) {
    case ERROR_CATEGORIES.RATE_LIMIT:
      return "AURA is experiencing high demand or rate limits right now. Please wait a moment and try again.";
    case ERROR_CATEGORIES.AUTH_FAILURE:
      return "Authentication failed while connecting to AURA intelligence core. Please check API key configuration.";
    case ERROR_CATEGORIES.TIMEOUT:
      return "Network timeout occurred while reaching AURA services. Please check your connection and try again.";
    case ERROR_CATEGORIES.INVALID_REQUEST:
      return "Invalid request format or parameters sent to AURA intelligence core.";
    case ERROR_CATEGORIES.SEARCH_FAILURE:
      return "Live web search failed or returned no reliable grounding sources. Unable to fulfill live web search request.";
    case ERROR_CATEGORIES.SERVER_ERROR:
    default:
      return sanitizeString(fallbackMsg) || "AURA encountered an internal service error while generating a response.";
  }
}
