...
export async function executeAuraPipeline({ message, messages }) {
  ...
  try {
    if (taskCategory === TASK_TYPES.DEEP_RESEARCH) {
      ...
      selectedSourceURLs = researchResult.sources.map((s) => s.url);
...
`.trim();
      ...
      searchRequestSuccess = response.searchSuccess && sources.length > 0;
      ...
      // STRICT FALLBACK: If live web search failed or returned 0 grounding sources, DO NOT answer from internal model memory.
